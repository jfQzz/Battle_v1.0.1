...
public class AppAdapter extends RecyclerView.Adapter<AppAdapter.Holder> {
    ...
    private List<AppInfoList.ItemsBean> mData;
    private LayoutInflater inflater;
    private ItemClick itemClick;

    public AppAdapter(Context mContext, ArrayList<AppInfoList.ItemsBean> mData) {
    ...
    public void onBindViewHolder(Holder holder, int position) {
        AppInfoList.ItemsBean itemsBean = mData.get(position);
        holder.ivAppPic.setImageURI(itemsBean.getImage());
        holder.tvAppName.setText(Html.fromHtml(itemsBean.getAppname()));
        String labels = itemsBean.getLabels().replaceAll(Constant.string.COMMA_SEPARATOR,"\t");
        String label = String.valueOf(Integer.parseInt(itemsBean.getSoftsize()) / 1024+"M\t\t");
        if(!TextUtils.isEmpty(labels)){
            label += labels;
        }
        holder.tvAppRemark.setText(label);
        holder.tvAppDesc.setText(Html.fromHtml(itemsBean.getDesc()));
