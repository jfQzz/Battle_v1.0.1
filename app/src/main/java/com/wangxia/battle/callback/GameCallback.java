 ...
 * Explain：
 */
public class GameCallback extends IBaseCacheCallback {
    private ICallback iCallback;
    private int type;

    public GameCallback(ICallback iCallback, int type) {
        super();
    ...
    public void onError(Call call, Exception e, int id) {
        ...
        if (!NetUtil.isNetAvailable(App.context) && !TextUtils.isEmpty(cache)) {
            iCallback.success(GsonUtil.getGson().fromJson(cache, AppInfo.class), type);
        }else {
            iCallback.error();
        }
        call.cancel();
    ...
    public void onResponse(String response, int id) {
        if (!TextUtils.isEmpty(response)) {
            iCallback.success(GsonUtil.getGson().fromJson(response, AppInfo.class), type);
        } else {
            iCallback.fail();
        }
    }
