 ...
 * Explain：
 */
public class EvaluateCallback extends IBaseCacheCallback {
    private ICallback iCallback;
    private int type;

    public EvaluateCallback(ICallback iCallback, int type) {
        super();
    ...
    public void onError(Call call, Exception e, int id) {
        ...
        if (!NetUtil.isNetAvailable(App.context) && !TextUtils.isEmpty(cache)) {
            iCallback.success(GsonUtil.getGson().fromJson(cache, EvaluateBean.class), type);
        }else {
            iCallback.error();
        }
        call.cancel();
    ...
    public void onResponse(String response, int id) {
        if (!TextUtils.isEmpty(response)) {
            iCallback.success(GsonUtil.getGson().fromJson(response, EvaluateBean.class), type);
        } else {
            iCallback.fail();
        }
    }
